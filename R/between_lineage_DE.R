...
branch_specific_genes <- function(cds, test_lineages, name, U = NULL, nknots = 6, parallel = F, p_cutoff = 0.05, FC_cutoff = 1){
  ...
  for(test_lineage in test_lineages){
    ...
    lineage_genes = get_lineage_genes(cds, test_lineage, genes = genes, U = U, nknots = nknots, parallel = parallel, p_cutoff = 1, FC_cutoff = 0, lineages = c(test_lineage, lineages[!(lineages %in% test_lineages)]))
