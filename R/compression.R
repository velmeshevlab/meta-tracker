...
#' into one object. Each lineage is compressed in a separate forked process on
#' Unix/macOS (or across a cluster if \code{cl} is a \code{makeCluster()}
#' object); Windows without a cluster runs serially. Each worker prints the
#' lineage it is compressing (with its PID) when it starts and finishes, so you
#' can follow progress across processes and see immediately if one stalls.
#'
#' Uses \code{mc.preschedule = FALSE}: a fresh process per lineage rather than
#' re-using one worker for a fixed chunk of lineages. Re-using a forked worker
#' across monocle3 \code{project2MST}/\code{order_cells} calls can deadlock after
#' the first lineage; a fresh fork per lineage avoids that and load-balances
#' lineages of very different sizes.
#'
...
compress_lineages <- function(cds, lineages = names(cds@lineages), N,
  ...
  worker <- function(lin) {
    t0 <- Sys.time()
    message(sprintf("[pid %d] compressing lineage '%s' ...", Sys.getpid(), lin))
    tmp <- compress_lineage(cds, lineage = lin, N = N, method = method,
                            ID = ID, progress = FALSE)
    message(sprintf("[pid %d] finished lineage '%s' (%s)", Sys.getpid(), lin,
  ...
  res <-
    if (inherits(cl, "cluster")) {
      parallel::parLapply(cl, lineages, worker)
    } else if (is.numeric(cl) && cl > 1 && .Platform$OS.type != "windows") {
      parallel::mclapply(lineages, worker, mc.cores = cl, mc.preschedule = FALSE)
    } else {
      lapply(lineages, worker)
    }
  names(res) <- lineages
